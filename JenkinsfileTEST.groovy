...
def imageName = "gazelle-analyser-api-test"
def imageTag ="0.1.0"
def dockerFile = "DockerfileTEST"
//}
...
node(testNode) {
    try{
        ...
        stage("Pulling ${imageName} image code") {
            git credentialsId: "${gitCredentials}", url: "${gitRepo}", branch: "${gitBranch}"
        }
        ...
        stage("Building ${imageName} image") {
            sshagent([sshCredentials]) {
                ...
                sh "${sshHeader} docker build -t ${dockerRegistryUrl}/${imagePath}/${imageName}:${imageTag} -f ${imageName}/${dockerFile} ${imageName}/."
